...
| `display_mode`         |  `string`  |               `environment`               | <ul><li>`always`: the segment is always displayed</li><li>`files`: the segment is only displayed when file `extensions` listed are present</li><li>`environment`: displays the segment when the environment is active</li><li>`context`: displays the segment when the environment is active or files are present</li></ul> |
